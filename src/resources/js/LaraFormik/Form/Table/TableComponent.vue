<template>
  <div class="shadow ring-1 ring-black ring-opacity-5 rounded-lg">
    ...
    <div class="flex p-4 flex-wrap items-center" v-if="label||action.href||$slots.actions">
      ...
      <div class="ml-16 flex gap-2">
        ...
        <Dropdown align="left" width="48" v-if="actionForm.selectedData?.length">
          ...
          <template #content>
            <button
                class="block w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 transition
                duration-150 ease-in-out hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
        ...
        <TextInput v-model="searchKeyword"
          ...
          <template #suffix>
            <XMarkIcon :class="[colors.close2]" class="size-5 font-bold ease-in-out duration-300"
                       @click="handleResetSearch"/>
          </template>
        </TextInput>
        <slot name="actions">
          <ButtonComponent :mode="mode" v-if="action?.href" :label="action.label" :href="action?.href"/>
        </slot>
            ...
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tr v-for="(it, ind) in tableItems" :key="ind">
                <td v-if="actions?.options"
                  ...
                  <div>
                    <Checkbox :value="it?.id" v-model="actionForm.selectedData"/>
                  </div>
                </td>
              ...
              <tr>
                <td :colspan="fields.length +(actions?.options?1:0)">
                  <div class="flex text-center flex-col justify-center py-10 gap-4">
                    ...
                    <div>
                      <slot name="actions">
                        <ButtonComponent :mode="mode" v-if="action?.href" :label="action.label"
                                         :href="action?.href"/>
                      </slot>
                    </div>
                  </div>
    ...
    <Pagination :mode="mode" :options="options"/>
